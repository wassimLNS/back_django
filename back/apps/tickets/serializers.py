...
class CreerTicketSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        centre = None
        if client.telephone:
            for c in CentreDistribution.objects.filter(actif=True):
                for prefix in c.prefixes_tel:
                    if client.telephone.startswith(prefix):
                        centre = c
                        break
                if centre:
                    break
        ...
        if not centre:
            raise serializers.ValidationError("Aucun centre trouvé pour votre numéro de téléphone.")
